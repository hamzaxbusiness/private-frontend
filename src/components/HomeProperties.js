  ...
  useEffect(() => {
    async function fetchCategories() {
      const data = await axios.get(
        "https://tweed-jacket-narwhal.cyclic.app/api/admin/getRandomProperties"
      );
              ...
              {properties.map((element, index) => (
                <SwiperSlide>
                  <Link to={`property-view/${element._id}`}>
                    <div className="relative overflow-hidden object-contain rounded-lg shadow-md shadow-gray-600 cursor-pointer grid content-center bg-gray-700 hover:bg-gray-800">
                      <div className="h-[200px]">
                        <img
                          className="w-[400px] h-[236px] rounded-md object-fill mix-blend-overlay "
